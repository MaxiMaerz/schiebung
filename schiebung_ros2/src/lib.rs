...
impl RosBuffer {
        ...
        let tf_subscriber = node.create_subscription::<TFMessage, _>(
            ...
            move |msg: TFMessage| {
                for transform in msg.transforms {
                    let stamp =
                            transform.header.stamp.sec as f64 + (transform.header.stamp.nanosec as f64) * 1e-9;
                    ...
                    let stamped_transform = StampedIsometry{isometry: isometry, stamp: stamp};
                    buffer_clone.lock().unwrap().update(source, target, stamped_transform, TransformType::Dynamic).unwrap();
                }
            },
        )?;
        let buffer_clone = Arc::clone(&buffer);
        let static_tf_subscriber = node.create_subscription::<TFMessage, _>(
            ...
            move |msg: TFMessage| {
                for transform in msg.transforms {
                    ...
                    let isometry = Isometry3::from_parts(trans, rot);
                    let stamped_transform = StampedIsometry{isometry: isometry, stamp: stamp};
                    buffer_clone.lock().unwrap().update(source, target, stamped_transform, TransformType::Static).unwrap();
                }
            },
        )?;
        Ok(Self {
    ...
    pub fn lookup_transform(&self, source: &str, target: &str, stamp: f64) -> Result<StampedIsometry, TfError> {
        self.buffer.lock().unwrap().lookup_transform(source.to_string(), target.to_string(), stamp)
    }
    pub fn lookup_latest_transform(&self, source: &str, target: &str) -> Result<StampedIsometry, TfError> {
        self.buffer.lock().unwrap().lookup_latest_transform(source.to_string(), target.to_string())
    }
